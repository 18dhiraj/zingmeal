...
const SavedPlanItem: React.FC<SavedPlanItemProps> = ({
  plan,
  meals,
  onRemove,
  isLoadingMeals,
    ...
    <Card className="shadow-md hover:shadow-lg transition-shadow bg-card flex flex-col">
      ...
      <CardFooter className="flex justify-end gap-2 border-t pt-4 mt-auto">
        <Button
          ...
          size="sm"
          onClick={viewPlan}
          disabled={isLoadingMeals || meals.length === 0}
        >
...
export default function FavoritesPage() {
  ...
  const loadSavedPlans = useCallback(async () => {
    ...
    const plansFromStorage = await getSavedMealPlans();
    setSavedPlans(plansFromStorage);

    if (plansFromStorage.length > 0) {
      for (const plan of plansFromStorage) {
        setIsLoadingPlanDetails((prev) => ({ ...prev, [plan.id]: true }));
        try {
          const fetchedMeals = plan.mealIds.length
            ? await fetchMealsByIds(plan.mealIds)
            : [];
          ...
          toast({
            title: "Error",
            description: `Could not load details for plan ${plan.name || plan.id.substring(0, 8)}.`,
            variant: "destructive",
          });
  ...
  const loadSavedIndividualMeals = useCallback(async () => {
    ...
    const mealIds = await getSavedIndividualMealIds();
    if (mealIds.length > 0) {
      try {
        ...
        toast({
          title: "Error",
          description: "Could not load details for saved individual meals.",
          variant: "destructive",
        });
  ...
  const overallLoading =
    isLoadingPlans ||
    isLoadingIndividualMeals ||
    Object.values(isLoadingPlanDetails).some((s) => s);
        ...
        <div className="space-y-10 mt-10">
          <div>
            <h2 className="text-2xl font-semibold text-primary mb-4 flex items-center gap-2">
              <Utensils className="w-6 h-6" /> Saved Meal Plans
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {savedPlans.map((plan) => (
              <SavedPlanItem
                ...
                isLoadingMeals={
                  isLoadingPlanDetails[plan.id] === undefined
                    ? true
                    : isLoadingPlanDetails[plan.id]
                }
              />
            ))}
          </div>
