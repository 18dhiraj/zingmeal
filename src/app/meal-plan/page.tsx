  ...
  useEffect(() => {
    ...
    const fetchAndSetMeals = async () => {
      if (mealIdsParam) {
        ...
        try {
          const saved = await isMealPlanSaved(mealIdArray);
          setIsCurrentPlanSaved(saved);
        } catch (err) {
          console.error("Failed to check if plan is saved:", err);
        }
        ...
        if (meals.length === 0 || mealIdsParam !== currentMealStateIds) {
          ...
          setError(null);
          try {
            const fetchedMeals = await fetchMealsByIds(mealIdArray);
            if (fetchedMeals && fetchedMeals.length === mealIdArray.length) {
              const orderedMeals = mealIdArray.map(id => fetchedMeals.find(m => m.id === id)).filter(Boolean) as Meal[];
    ...
    fetchAndSetMeals();
  }, [searchParams, toast]); // Add toast if used inside fetch
  ...
  const handleSwapMealInPlan = useCallback(async (mealIdToSwap: string, mealIndexInPlan?: number) => {
    ...
    try {
      ...
      if (nextMeal) {
        ...
        const saved = await isMealPlanSaved((newMealsArray.map(m => m.id))); // Check if new combination is saved
        setIsCurrentPlanSaved(saved);
